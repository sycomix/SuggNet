...
# save
ica1.save('data/ica/ecg-ica.fif')
np.save('data/ica/ecg_template.npy', np.array(ecg_inds))

# eog template from subject 2 that has two typical eog component
...
raw2.load_data().set_eeg_reference(['M1', 'M2'])
raw2.filter(1, 30)
pos = _make_montage()
raw2.set_montage(pos)

ica2 = mne.preprocessing.ICA(40, max_iter='auto', random_state=42)
ica2.fit(raw2)
eog_inds = ica2.find_bads_eog(raw2, ch_name=['EOG1', 'EOG2', 'Fp1', 'Fp2', 'Fpz'])
# save
ica2.save('data/ica/eog-ica.fif')
np.save('data/ica/eog_template.npy', np.array(eog_inds))
